...
const companylist = async (req, res) => {
    ...
    var id = req.query.id;
    var sql = `select  a.exam_id,exam_name from user_answers a, exam b where a.exam_id=b.exam_id and a.user_id=${id} limit ${offset},${limit};`
    
    let [query] = await db.query(sql);

    var sql2 = `select  a.exam_id,exam_name from user_answers a, exam b where a.exam_id=b.exam_id and a.user_id=${id};`;
    let [query2] = await db.query(sql2);
    ...
    res.render('companylist', { data: query  , count ,limit , curpage ,id});
